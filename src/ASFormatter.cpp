...
//  init();
  ...
  void ASFormatter::init(ASSourceIterator* si)
  {
    ...
    initContainer(preBracketHeaderStack, new std::vector<const std::string*>);
    initContainer(parenStack, new std::vector<std::int32_t>);
    initContainer(structStack, new std::vector<bool>);
    parenStack->push_back(0);               // parenStack must contain this default entry
    initContainer(bracketTypeStack, new std::vector<BracketType>);
    ...
    currentLineFirstBracketNum = std::string::npos;
...
//      std::int32_t x = 1;
      ...
      if(isImmediatelyPostHeader)
      {
        ...
        if(currentChar != '{' && shouldAddBrackets)
        {
          ...
          {
            std::size_t firstText = currentLine.find_first_not_of(" \t");
            assert(firstText != std::string::npos);

            if((std::int32_t) firstText == charNum)
            {
      ...
      if(isPotentialHeader && !isInTemplate)
      {
        ...
        if(newHeader != nullptr)
        {
              ...
              && !shouldBreakElseIfs && !isCharImmediatelyPostLineComment)
          {
            ...
            std::size_t start = formattedLine.length() >= 6 ? formattedLine.length() - 6 : 0;

            if(formattedLine.find("else", start) != std::string::npos
                && formattedLine.find("#else", start) == std::string::npos)
            {
              appendSpacePad();
              isInLineBreak = false;
            }
          ...
          if(shouldPadHeader
              ...
              && charNum < (std::int32_t) currentLine.length() - 1 && !isWhiteSpace(currentLine[charNum + 1]))
          {
            appendSpacePad();
          }
          ...
          || (currentChar == ',' && shouldPadOperators))
      {
        ...
        if(charNum + 1 < (std::int32_t) currentLine.length())
        {
      ...
      if(newHeader != nullptr && newHeader->length() > 1)
      {
        appendSequence(*newHeader);
        goForward(newHeader->length() - 1);
        continue;
    ...
    std::size_t readyFormattedLineLength = trim(readyFormattedLine).length();
   ...
   */
  void ASFormatter::goForward(std::int32_t i)
  {
    while(--i >= 0)
    {
      getNextChar();
  ...
  {
    char ch = ' ';
    std::size_t peekNum = currentLine.find_first_not_of(" \t", charNum + 1);
   ...
   * @return     is before a comment or line-comment.
   */
  bool ASFormatter::isBeforeAnyLineEndComment(std::int32_t startPos) const
  {
    bool foundLineEndComment = false;
    std::size_t peekNum = currentLine.find_first_not_of(" \t", startPos + 1);
    ...
    if(peekNum != std::string::npos)
    {
      ...
      else if(currentLine.compare(peekNum, 2, "/*") == 0)
      {
        ...
        std::size_t endNum = currentLine.find("*/", peekNum + 2);

        if(endNum != std::string::npos)
        {
          std::size_t nextChar = currentLine.find_first_not_of(" \t", endNum + 2);
   ...
   */
  bool ASFormatter::isBeforeMultipleLineEndComments(std::int32_t startPos) const
  {
  ...
  {
    std::size_t len = currentLine.length();
    std::size_t indent = getIndentLength();
    charNum = 0;
    ...
    if(isInExecSQL)
    {
      ...
      for(i = 0; i < currentLine.length(); i++)
      {
        ...
        if(currentLine[i] == '\t')
        {
          std::size_t numSpaces = indent - ((tabCount_ + i) % indent);
    ...
    for(charNum = 0; isWhiteSpace(currentLine[charNum]) && charNum + 1 < (std::int32_t) len; charNum++)
    {
    ...
    else if(isSequenceReached("{"))
    {
      ...
      std::size_t firstText = currentLine.find_first_not_of(" \t", charNum + 1);
    ...
    else if(isWhiteSpace(currentLine[charNum]) && !(charNum + 1 < (std::int32_t) currentLine.length()))
    {
      lineIsEmpty = true;
    }
  }
  ...
  {
    std::int32_t len = formattedLine.length();
  ...
  {
    std::int32_t len = currentLine.length();
  ...
  BracketType ASFormatter::getBracketType()
  {
    ...
    std::int32_t foundOneLineBlock = isOneLineBlockReached(currentLine, charNum);
  ...
  bool ASFormatter::isPointerOrReference() const
  {
        ...
        && isLegalNameChar(nextChar))
    {
      ...
      std::size_t nextNum = currentLine.find_first_of("=;)]", charNum + 1);

      if(nextNum != std::string::npos && currentLine[nextNum] == '=')
      {
        return true;
      }
    ...
    if(nextChar == '-'
        || nextChar == '+')
    {
      std::size_t nextNum = currentLine.find_first_not_of(" \t", charNum + 1);
  ...
  bool ASFormatter::isDereferenceOrAddressOf() const
  {
    ...
    // check for **
    if(currentChar == '*'
        && (std::int32_t) currentLine.length() > charNum
        && currentLine[charNum + 1] == '*')
    {
      ...
      if((std::int32_t) currentLine.length() < charNum + 2)
      {
        return true;
      }
    ...
    if(charNum == (std::int32_t) currentLine.find_first_not_of(" \t"))
    {
      return true;
    }

    std::size_t nextChar = currentLine.find_first_not_of(" \t", charNum + 1);
  ...
  bool ASFormatter::isPointerOrReferenceCentered() const
  {
    ...
    std::int32_t prNum = charNum;
    std::int32_t lineLength = (std::int32_t) currentLine.length();
  ...
  bool ASFormatter::isInExponent() const
  {
    ...
    std::int32_t formattedLineLength = formattedLine.length();
  ...
  bool ASFormatter::isNonInStatementArrayBracket() const
  {
    ...
    if(currentLineBeginsWithBracket
        && charNum == (std::int32_t) currentLineFirstBracketNum
        && nextChar != '}')
    {
      returnVal = true;
    }
    ...
    // trace
    //if (isNonInStatementArray)
    //  cout << traceLineNumber << " " << 'x' << std::endl;
    //else
    //  cout << traceLineNumber << " " << ' ' << std::endl
   ...
   */
  std::int32_t ASFormatter::isOneLineBlockReached(std::string& line, std::int32_t startChar) const
  {
    assert(line[startChar] == '{');

    bool isInComment_ = false;
    bool isInQuote_ = false;
    std::int32_t bracketCount = 1;
    std::int32_t lineLength = line.length();
    char quoteChar_ = ' ';
    char ch = ' ';
    char prevCh = ' ';

    for(std::int32_t i = startChar + 1; i < lineLength; ++i)
    {
      ch = line[i];
   ...
   */
  bool ASFormatter::isNextWordSharpNonParenHeader(std::int32_t startChar) const
  {
   ...
   */
  bool ASFormatter::isNextCharOpeningBracket(std::int32_t startChar) const
  {
    bool retVal = false;
    std::string nextText = peekNextText(currentLine.substr(startChar));
   ...
   */
  std::string ASFormatter::peekNextText(const std::string& firstLine, bool endOnEmptyLine /*false*/, bool shouldReset /*false*/) const
  {
    bool isFirstLine = true;
    bool needReset = shouldReset;
    std::string nextLine_ = firstLine;
    std::size_t firstChar = std::string::npos;
  ...
  void ASFormatter::adjustComments(void)
  {
    ...
    {
      std::size_t endNum = currentLine.find("*/", charNum + 2);
      ...
      if(currentLine.find_first_not_of(" \t", endNum + 2) != std::string::npos)
      {
        return;
      }
    }

    std::size_t len = formattedLine.length();
    ...
    if(spacePadNum < 0)
    {
      std::int32_t adjust = -spacePadNum;          // make the number positive
    ...
    {
      std::int32_t adjust = spacePadNum;
      std::size_t lastText = formattedLine.find_last_not_of(' ');
  ...
  void ASFormatter::appendCharInsideComments(void)
  {
    ...
    std::size_t end = formattedLineCommentNum;
    std::size_t beg = formattedLine.find_last_not_of(" \t", end - 1);
  ...
  void ASFormatter::padOperators(const std::string* newOperator)
  {
    ...
    if(shouldPad
        ...
        && !(newOperator == &AS_QUESTION && isSharpStyle() // check for C# nullable type (e.g. std::int32_t?)
  ...
  void ASFormatter::formatPointerOrReference(void)
  {
    ...
    std::int32_t pa = pointerAlignment;
    std::int32_t ra = referenceAlignment;
    std::int32_t itemAlignment = (currentChar == '*') ? pa : ((ra == REF_SAME_AS_PTR) ? pa : ra);

    // check for cast
    char peekedChar = peekNextChar();

    if(currentChar == '*'
        && (std::int32_t) currentLine.length() > charNum + 1
        && currentLine[charNum + 1] == '*')
    {
      std::size_t nextChar = currentLine.find_first_not_of(" \t", charNum + 2);
    ...
    {
      std::size_t prevCh = formattedLine.find_last_not_of(" \t");

      if(prevCh == std::string::npos)
      {
        prevCh = 0;
      }
      ...
      else
      {
        ...
        std::string charSave = formattedLine.substr(prevCh + 1, 1);
      ...
      if(charNum < (std::int32_t) currentLine.length() - 1
          && !isWhiteSpace(currentLine[charNum + 1])
          && currentLine[charNum + 1] != ')')
      {
        appendSpacePad();
    ...
    else if(itemAlignment == PTR_ALIGN_MIDDLE)
    {
      ...
      std::size_t wsBefore = currentLine.find_last_not_of(" \t", charNum - 1);
      ...
      // adjust for **
      std::string sequenceToInsert = currentChar == '*' ? "*" : "&";
      ...
      std::size_t charNumSave = charNum;
      ...
      if((std::int32_t) currentLine.find_first_not_of(" \t", charNum + 1) > charNum)
      {
        ...
        for(std::size_t i = charNum + 1; i < currentLine.length() && isWhiteSpace(currentLine[i]); i++)
        {
          goForward(1);
      ...
      std::size_t wsAfter = currentLine.find_first_not_of(" \t", charNumSave + 1);
      ...
      else
      {
        ...
        {
          std::size_t charsToAppend = (2 - (wsBefore + wsAfter));
    ...
    else if(itemAlignment == PTR_ALIGN_NAME)
    {
      ...
      // if this is not the last thing on the line
      if(!isBeforeAnyComment()
          && (std::int32_t) currentLine.find_first_not_of(" \t", charNum + 1) > charNum)
      {
        // goForward() to convert tabs to spaces, if necessary,
        // and move following characters to preceding characters
        // this may not work every time with tab characters
      ...
      if(startNum != std::string::npos && isAfterScopeResolution)
      {
        std::size_t lastText = formattedLine.find_last_not_of(" \t");
  ...
  void ASFormatter::padParens(void)
  {
    ...
    std::int32_t spacesOutsideToDelete = 0;
    std::int32_t spacesInsideToDelete = 0;
    ...
    if(currentChar == '(')
    {
      ...
      if(shouldUnPadParens)
      {
        ...
        std::size_t i = formattedLine.find_last_not_of(" \t");
        ...
        if(i != std::string::npos)
        {
          ...
          else
          {
            ...
            if(prevWordH != nullptr)
            {
              ...
              // trace
              //cout << traceLineNumber << " " << *prevWordH << std::endl;
            }
            else if(prevWord == "return")   // don't unpad return statements
            {
              prevIsParenHeader = true;
              // trace
              //cout << traceLineNumber << " " << prevWord << std::endl;
            }
      ...
      if(shouldUnPadParens)
      {
        std::size_t j = currentLine.find_first_not_of(" \t", charNum + 1);
      ...
      //    cout << traceLineNumber << " " << spacesOutsideToDelete << '(' << spacesInsideToDelete << std::endl;
    }
    else if(currentChar == ')')
    {
      spacesOutsideToDelete = 0;
      ...
      // trace
      //if(spacesInsideToDelete > 0)
      //  cout << traceLineNumber << " " << spacesInsideToDelete << ')' << 0 << std::endl;
    }
  ...
  void ASFormatter::formatClosingBracket(BracketType bracketType)
  {
    ...
    if(shouldAttachClosingBracket)
    {
      ...
      if((isEmptyLine(formattedLine)      // if a blank line preceeds this
          || isCharImmediatelyPostLineComment
          || isCharImmediatelyPostComment
          || (isImmediatelyPostPreprocessor && (std::int32_t) currentLine.find_first_not_of(" \t") == charNum)
...
//          && !isImmediatelyPostEmptyBlock)    /* removed 9/5/10 */      // this '}' does not immediately follow an empty block
    ...
    if(shouldBreakBlocks && currentHeader != nullptr && parenStack->back() == 0)
    {
          ...
          else
          {
            ...
            else
            {
                  ...
                  && !isBracketType(bracketType, SINGLE_LINE_TYPE))
              {
                ...
                if(currentLineBeginsWithBracket
                    && (std::int32_t)currentLineFirstBracketNum == charNum)
                {
                  shouldBreakLineAtNextChar = true;
                }
              }
  ...
  void ASFormatter::formatRunIn()
  {
    ...
    {
      std::int32_t indent = getIndentLength();
  ...
  /**
   * delete a bracketTypeStack std::vector object
   * BracketTypeStack did not work with the DeleteContainer template
   */
  void ASFormatter::deleteContainer(std::vector<BracketType>* &container)
  {
  ...
  /**
   * initialize a BracketType std::vector object
   * BracketType did not work with the DeleteContainer template
   */
  void ASFormatter::initContainer(std::vector<BracketType>* &container, std::vector<BracketType>* value)
  {
    if(container != nullptr)
    {
  ...
  void ASFormatter::convertTabToSpaces()
  {
    ...
    std::size_t indent = getIndentLength();
    std::size_t numSpaces = indent - ((tabIncrementIn + charNum) % indent);
  ...
  */
   ...
   */
  void ASFormatter::checkForHeaderFollowingComment(const std::string& firstLine)
  {
    assert(isInComment || isInLineComment);
    // this is called ONLY IF shouldBreakBlocks is TRUE.
    assert(shouldBreakBlocks);
    // look ahead to find the next non-comment text
    std::string nextText = peekNextText(firstLine, true);
    ...
    const std::string* newHeader = ASBeautifier::findHeader(nextText, 0, headers);
  ...
  void ASFormatter::processPreprocessor()
  {
    ...
    const std::int32_t preproc = charNum + 1;
    ...
    else if(currentLine.compare(preproc, 4, "else") == 0)
    {
      ...
      {
        std::int32_t addedPreproc = bracketTypeStack->size() - preprocBracketTypeStackSize;

        for(std::int32_t i = 0; i < addedPreproc; i++)
        {
          bracketTypeStack->pop_back();
        }
      }
  ...
  bool ASFormatter::commentAndHeaderFollows()
  {
    ...
    std::string nextLine_ = sourceIterator->peekNextLine();
    std::size_t firstChar = nextLine_.find_first_not_of(" \t");
    ...
    std::string nextText = peekNextText(nextLine_, false, true);
  ...
  bool ASFormatter::isCurrentBracketBroken() const
  {
    ...
    std::size_t bracketTypeStackEnd = bracketTypeStack->size() - 1;
    ...
    {
      if(currentLineBeginsWithBracket
          && (std::int32_t)currentLineFirstBracketNum == charNum)    // lineBeginsWith('{')
      {
        breakBracket = true;
      }
    }
  ...
  void ASFormatter::formatCommentBody()
  {
    ...
    else
    {
      ...
      while(charNum + 1 < (std::int32_t) currentLine.length()
            && currentLine[charNum + 1] != '\t'
            && currentLine.compare(charNum + 1, 2, "*/") != 0)
      {
        currentChar = currentLine[++charNum];
   ...
   * i.e "} else" changed to "} \n else".
   */
  std::int32_t ASFormatter::getNextLineCommentAdjustment()
  {
    ...
    std::size_t lastBracket = currentLine.rfind('}', charNum - 1);
...
// for console build only
   ...
   */
  std::int32_t ASFormatter::getCurrentLineCommentAdjustment()
  {
    assert(foundClosingHeader && previousNonWSChar == '}');
   ...
   */
  std::string ASFormatter::getPreviousWord(const std::string& line, std::int32_t currPos) const
  {
    // get the last legal word (may be a number)
    if(currPos == 0)
    {
    ...
    std::size_t end = line.find_last_not_of(" \t", currPos - 1);
    ...
    std::int32_t start;          // start of the previous word
  ...
  void ASFormatter::isLineBreakBeforeClosingHeader()
  {
    ...
    else if(bracketFormatMode == NONE_MODE)
    {
      ...
      else
      {
        ...
        std::size_t i = currentLine.find_first_not_of(" \t");
    ...
    else
    {
      ...
      else
      {
        ...
        std::int32_t previousLineIsOneLineBlock = 0;
        std::size_t firstBracket = findNextChar(formattedLine, '{');
  ...
  bool ASFormatter::addBracketsToStatement()
  {
    ...
    std::size_t nextSemiColon = charNum;
   ...
   */
  std::size_t ASFormatter::findNextChar(std::string& line, char searchChar, std::int32_t searchStart /*0*/)
  {
    ...
    for(i = searchStart; i < line.length(); i++)
    {
      ...
      if(line.compare(i, 2, "/*") == 0)
      {
        std::size_t endComment = line.find("*/", i + 2);
      ...
      if(line[i] == '\'' || line[i] == '\"')
      {
        ...
        while(i < line.length())
        {
          std::size_t endQuote = line.find(quote, i + 1);
   ...
   */
  bool ASFormatter::isStructAccessModified(std::string&  firstLine, std::size_t index) const
  {
    ...
    while(sourceIterator->hasMoreLines())
    {
      ...
      // parse the line
      for(std::size_t i = 0; i < nextLine_.length(); i++)
      {
   ...
   */
  bool ASFormatter::isExecSQL(std::string&  line, std::size_t index) const
  {
    ...
    std::size_t index2 = index + word.length();
  ...
  void ASFormatter::trimContinuationLine()
  {
    ...
    {
      std::size_t i;
      std::size_t continuationIncrementIn = 0;
      ...
      if((std::int32_t) continuationIncrementIn == tabIncrementIn)
      {
        charNum = i;
      }
      else
      {
        ...
        std::string newLine;
        std::int32_t leadingChars = 0;

        if((std::int32_t) leadingSpaces > tabIncrementIn)
        {
  ...
  bool ASFormatter::isImmediatelyPostCast() const
  {
    ...
    std::size_t lastChar = currentLine.find_last_not_of(" \t", paren - 1);
  ...
  void ASFormatter::checkIfTemplateOpener()
  {
    ...
    std::int32_t parenDepth_ = 0;
    std::int32_t maxTemplateDepth = 0;
    templateDepth = 0;

    for(std::size_t i = charNum; i < currentLine.length(); i++)
    {
      ...
      else if(currentChar_ == ','        // comma,     e.g. A<std::int32_t, char>
              || currentChar_ == '&'    // reference, e.g. A<std::int32_t&>
              || currentChar_ == '*'    // pointer,   e.g. A<std::int32_t*>
  ...
  {
    for(std::size_t i = 0; i < currentLine_.length(); i++)
   ...
   * @return   checksumIn.
   */
  std::size_t ASFormatter::getChecksumIn() const
  {
    return checksumIn;
  }
  ...
  {
    for(std::size_t i = 0; i < beautifiedLine.length(); i++)
   ...
   * @return   checksumOut.
   */
  std::size_t ASFormatter::getChecksumOut() const
  {
    return checksumOut;
  }
   ...
   * If zero all is okay.
   */
  std::int32_t ASFormatter::getChecksumDiff() const
  {
    return checksumOut - checksumIn;
  }

// for unit testing
  std::int32_t ASFormatter::getFormatterFileType() const
  {
