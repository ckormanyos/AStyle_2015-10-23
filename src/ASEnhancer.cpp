...
//  init();
   ...
   */
  void ASEnhancer::init(std::int32_t fileType,
                        std::int32_t _indentLength,
   ...
   */
  void ASEnhancer::enhance(std::string& line, bool isInPreprocessor, bool isInSQL)
  {
    ...
    for(std::size_t i = 0; i < line.length(); i++)
    {
      ...
      // if we have reached this far then we are NOT in a comment or std::string of special characters
   ...
   */
  std::size_t ASEnhancer::findCaseColon(std::string&  line, std::size_t caseIndex) const
  {
    std::size_t i = caseIndex;
   ...
   */
  std::int32_t ASEnhancer::indentLine(std::string&  line, std::int32_t indent) const
  {
    if(line.length() == 0
        && ! emptyLineFill)
    {
   ...
   */
  bool ASEnhancer::isBeginDeclareSectionSQL(std::string&  line, std::size_t index) const
  {
    std::string word;
    std::size_t hits = 0;
    std::size_t i;
   ...
   * @return              true if a hit.
   */
  bool ASEnhancer::isEndDeclareSectionSQL(std::string&  line, std::size_t index) const
  {
    std::string word;
    std::size_t hits = 0;
    std::size_t i;
   ...
   */
  bool ASEnhancer::isOneLineBlockReached(std::string& line, std::int32_t startChar) const
  {
    assert(line[startChar] == '{');

    bool isInComment_ = false;
    bool isInQuote_ = false;
    std::int32_t _bracketCount = 1;
   ...
   */
  std::size_t ASEnhancer::processSwitchBlock(std::string& line, std::size_t index)
  {
    std::size_t i = index;
    bool isPotentialKeyword = isCharPotentialHeader(line, i);
    ...
    if(line[i] == '}')                                  // if close bracket
    {
      ...
      {
        std::int32_t lineUnindent = sw.unindentDepth;
   ...
   */
  std::int32_t ASEnhancer::unindentLine(std::string&  line, std::int32_t unindent) const
  {
    std::size_t whitespace = line.find_first_not_of(" \t");
    ...
    std::size_t charsToErase;                    // number of chars to erase
