...
#ifdef _WIN32
    std::size_t lcid = GetUserDefaultLCID();
...
// digital mars doesn't have these
  const std::size_t SUBLANG_CHINESE_MACAU = 5;
  const std::size_t LANG_HINDI = 57;
#endif
...
// language ID http://msdn.microsoft.com/en-us/library/ee797784%28v=cs.20%29.aspx
  ...
  void ASLocalizer::setLanguageFromLCID(std::size_t lcid)
...
//       Same with LCIDToLocaleName().
  {
    ...
    std::size_t lang    = PRIMARYLANGID(LANGIDFROMLCID(m_lcid));
    std::size_t sublang = SUBLANGID(LANGIDFROMLCID(m_lcid));

    // find language in the wlc table
    std::size_t count = sizeof(wlc) / sizeof(wlc[0]);
...
//
// the language std::string has the following form
//
...
//      de_DE.iso88591@euro
  {
    // the constants describing the format of lang_LANG locale std::string
    static const std::size_t LEN_LANG = 2;
    static const std::size_t LEN_SUBLANG = 2;
    ...
    {
      std::string subLang = langStr.substr(LEN_LANG + 1, LEN_SUBLANG);
...
// Add a std::string pair to the translation std::vector.
  {
...
// Convert wchar_t to a multibyte std::string using the currently assigned locale.
// Return an empty std::string if an error occurs.
  {
    static bool msgDisplayed = false;
    // get length of the output excluding the nullptr and validate the parameters
    std::size_t mbLen = wcstombs(nullptr, wideStr.c_str(), 0);

    if(mbLen == std::string::npos)
    {
      if(!msgDisplayed)
      {
        fprintf(stderr, "\n%s\n\n", "Cannot convert to multi-byte std::string, reverting to English");
        msgDisplayed = true;
      }
    ...
    if(mbStr == nullptr)
    {
      if(!msgDisplayed)
      {
        fprintf(stderr, "\n%s\n\n", "Bad memory alloc for multi-byte std::string, reverting to English");
        msgDisplayed = true;
      }
  ...
  std::size_t Translation::getTranslationVectorSize() const
// Return the translation std::vector size.  Used for testing.
  {
    return m_translation.size();
  }

  bool Translation::getWideTranslation(const std::string& stringIn, std::wstring& wideOut) const
// Get the wide translation std::string. Used for testing.
  {
    std::size_t i;
...
// Translate a std::string.
// Return a static std::string instead of a member variable so the method can have a "const" designation.
// This allows "settext" to be called from a "const" method.
  {
    static std::string mbTranslation;
    mbTranslation.clear();

    for(std::size_t i = 0; i < m_translation.size(); i++)
    {
      if(m_translation[i].first == stringIn)
      {
...
// build the translation std::vector in the Translation base class
  {
    addPair("Formatted  %s\n", L"Formaté    %s\n");  // should align with unchanged
    addPair("Unchanged  %s\n", L"Inchangée  %s\n");  // should align with formatted
    addPair("Directory  %s\n", L"Répertoire  %s\n");
